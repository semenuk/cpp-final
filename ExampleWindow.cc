...
void ExampleWindow::setup()
{
	...
	// Описывает к какому матричному стеку применять операции
	// GL_PROJECTION - Применяет последующие матричные операции к стеку матриц проекции
	glMatrixMode(GL_PROJECTION);
	// Устанавливает перспективу
	gluPerspective(45., double(width()) / double(height()), 0.01, 20.0);
	// Описывает к какому матричному стеку применять операции
	// GL_MODELVIEW - Применяет последующие матричные операции к стеку матриц вида модели
...
void ExampleWindow::render()
{
	...
	// Разграничивает вершины примитива или группы подобных примитивов
	// GL_QUARDS - Считает каждую группу из четырех вершин независимым четырехугольником.
	glBegin(GL_QUADS);
